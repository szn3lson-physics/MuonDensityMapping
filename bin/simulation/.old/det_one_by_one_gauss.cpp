...
const std::string OUTPUT_RESULTS = "/home/kacper/MuonDensityMapping/bin/simulation/simulation_output/distance_analysis/distance_one_by_one_analysis_gauss.txt";
const std::string OUTPUT_FOLDER = "/home/kacper/MuonDensityMapping/bin/simulation/simulation_output/distance_analysis_gauss/";
const std::string OUTPUT_BEST_BIN_ANALYSIS = "/home/kacper/MuonDensityMapping/bin/simulation/simulation_output/distance_analysis/best_radius_bin_analysis_gauss.txt";
...
TestResult evaluate_radius(double current_radius_m, const std::vector<int>& real_data, const std::vector<double>& time_data, int total_observed, unsigned char* img_data, int width, int height) {
    printf("\n============================================================\n");
    printf("--- Physics scanning and Chi2 test for Gauss Radius R0 = %.2f m ---\n", current_radius_m);
    printf("============================================================\n");
    fflush(stdout);
                    ...
                    for (int s = 0; s <= max_map_steps; ++s) {
                        ...
                        if (xi >= 0 && xi < width && yi >= 0 && yi < height) {
                            ...
                            if (is_rock) {
                                ...
                                // ZMIANA: Model Gaussa dla zanikania wpływu pustych przestrzeni
                                double visibility_weight = std::exp(-(r * r) / (current_radius_m * current_radius_m)); 
    ...
    // ZMIANA: Zapis z nowym sufixem gauss
    char filename[512];
    snprintf(filename, sizeof(filename), "%s%.2f_metrow_gauss.txt", OUTPUT_FOLDER.c_str(), current_radius_m);
...
int main() {
    ...
    printf("\n============================================================\n");
    printf("FINAL ANSWER: OPTIMAL GAUSSIAN RADII (R0)\n");
    printf("============================================================\n");
    ...
    printf("\n============================================================\n");
    printf("DETAILED BIN-BY-BIN ANALYSIS FOR BEST GAUSS RADIUS: %.2f m\n", best_radius);
    printf("============================================================\n");
                    ...
                    for (int s = 0; s <= max_map_steps2; ++s) {
                        ...
                        if (xi >= 0 && xi < width && yi >= 0 && yi < height) {
                            ...
                            if (is_rock) {
                                ...
                                // ZMIANA: Model Gaussa również w powtórnym przeliczeniu dla najlepszego R0
                                double visibility_weight = std::exp(-(r * r) / (best_radius * best_radius)); 
                                horiz_rock_m += STEP_SIZE_M * (1.0 - visibility_weight);
                            }
                        } else {
                            horiz_rock_m += STEP_SIZE_M;
