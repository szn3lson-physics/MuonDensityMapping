...
const std::string OUTPUT_RESULTS = "D:\\MuonDensityMapping\\bin\\simulation\\simulation_output\\distance_one_by_one_analysis.txt";
...
TestResult evaluate_radius(double current_radius_m, const std::vector<int>& real_data, const std::vector<double>& time_data, int total_observed, unsigned char* img_data, int width, int height, unsigned int master_seed) {
    ...
    int bar_width = 50; // Rozmiar paska ładowania
    ...
    printf("\nRay-tracing finished for this radius.\n");
    fflush(stdout);
...
int main() {
    ...
    // ================= PHASE 1: LINEAR SCANNING =================
    printf("\n2. Starting full scan from %.1fm to %.1fm with step %.1fm...\n", MIN_SCAN_RADIUS_M, MAX_SCAN_RADIUS_M, SCAN_STEP_M);
    ...
    printf("\n============================================================\n");
    printf("FINAL ANSWER: OPTIMAL RADII (Closest to Red. Chi2 = 1.0)\n");
    printf("============================================================\n");
    ...
    printf("------------------------------------------------------------\n");
    printf("Full scan history (0-100m) saved to file: %s\n", OUTPUT_RESULTS.c_str());
