...
const std::string OUTPUT_RESULTS = "/home/kacper/MuonDensityMapping/bin/simulation/simulation_output/distance_analysis_prof/distance_one_by_one_analysis.txt";
const std::string OUTPUT_FOLDER = "/home/kacper/MuonDensityMapping/bin/simulation/simulation_output/distance_analysis_prof/";
const std::string OUTPUT_BEST_BIN_ANALYSIS = "/home/kacper/MuonDensityMapping/bin/simulation/simulation_output/distance_analysis_prof/best_radius_bin_analysis.txt";

const double MIN_SCAN_RADIUS_M = 30.0;  
const double MAX_SCAN_RADIUS_M = 30.0; 
...
// --- NOWY PARAMETR ---
// Szerokość filtra wygładzającego. 3 to lekkie muśnięcie. 
// Ustaw na 1, jeśli chcesz zupełnie surową, poszarpaną fizykę.
...
std::vector<double> get_bartlett_kernel(int M) {
    if (M <= 1) return {1.0}; // Brak wygładzania
...
TestResult evaluate_radius(double current_radius_m, const std::vector<int>& real_data, const std::vector<double>& time_data, int total_observed, unsigned char* img_data, int width, int height) {
    printf("\n============================================================\n");
    printf("--- Physics scanning and Chi2 test for Radius R = %.2f m ---\n", current_radius_m);
    printf("============================================================\n");
    fflush(stdout);
    ...
    // Smoothing (POPRAWIONE)
    std::vector<double> kernel = get_bartlett_kernel(SMOOTHING_WINDOW);
    std::vector<double> expected_1deg(180, 0.0);
    int half_k = kernel.size() / 2; // Dynamiczne obliczanie połówki filtra
    
    for (int i = 0; i < 180; ++i) {
        double smoothed = 0.0;
        for (int j = 0; j < (int)kernel.size(); ++j) {
            smoothed += raw_flux_360[(i + j - half_k + 360) % 360] * kernel[j];
            smoothed += raw_flux_360[(i + 180 + j - half_k + 360) % 360] * kernel[j];
        }
    ...
    char filename[512];
    snprintf(filename, sizeof(filename), "%s%.2f_metrow.txt", OUTPUT_FOLDER.c_str(), current_radius_m);
...
int main() {
    ...
    printf("\n2. Starting direct theoretical scan from %.1fm to %.1fm with step %.1fm...\n", MIN_SCAN_RADIUS_M, MAX_SCAN_RADIUS_M, SCAN_STEP_M);
    ...
    printf("\n============================================================\n");
    printf("FINAL ANSWER: OPTIMAL RADII (Direct Theory vs Data)\n");
    printf("============================================================\n");
    ...
    printf("\n============================================================\n");
    printf("DETAILED BIN-BY-BIN ANALYSIS FOR BEST RADIUS: %.2f m\n", best_radius);
    printf("============================================================\n");

    // Recompute expected_1deg 
    double max_az2 = std::atan(W / L);
    double max_el2 = std::atan(H / L);
    std::vector<double> az_valid2, el_valid2, weights_valid2;
    double step_az_el = PIXEL_SIZE / best_radius;
    ...
    int num_steps2 = static_cast<int>(std::round(best_radius / STEP_SIZE_M));

    printf("Recomputing ray-tracing for R = %.2f m...\n", best_radius);
    for (int angle_deg = 0; angle_deg < 180; ++angle_deg) {
        for (int side = 0; side < 2; ++side) {
            double rad = (angle_deg + side * 180) * (M_PI / 180.0);
            double eff_intensity_num = 0.0, weight_sum = 0.0;
    ...
    // Smoothing & Normalization (POPRAWIONE)
    std::vector<double> kernel2 = get_bartlett_kernel(SMOOTHING_WINDOW);
    std::vector<double> expected_1deg2(180, 0.0);
    int half_k2 = kernel2.size() / 2;
    
    for (int i = 0; i < 180; ++i) {
        double smoothed = 0.0;
        for (int j = 0; j < (int)kernel2.size(); ++j) {
            smoothed += raw_flux_360b[(i + j - half_k2 + 360) % 360] * kernel2[j];
            smoothed += raw_flux_360b[(i + 180 + j - half_k2 + 360) % 360] * kernel2[j];
        }
