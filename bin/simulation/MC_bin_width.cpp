...
const std::string OUTPUT_RESULTS = "D:\\MuonDensityMapping\\bin\\simulation\\simulation_output\\bin_analysis.txt";
...
// Function calculating p-value from Chi-square distribution
...
int main() {
    printf("============================================================\n");
    printf("PHASE 1: PHYSICAL CALCULATIONS AND 3D GEOMETRY (R = %.1fm)\n", TEST_RADIUS_M);
    printf("============================================================\n");
    ...
    // ==========================================================
    // INITIALIZATION OF MONTE CARLO GENERATOR (MASTER SEED)
    // ==========================================================
    ...
    printf("Generating ray matrix...\n");
    fflush(stdout);

    for (int angle_deg = 0; angle_deg < 180; ++angle_deg) {
        double results_h[2], results_i[2], results_3d[2];
        for (int side = 0; side < 2; ++side) {
            double rad = (angle_deg + side * 180) * (M_PI / 180.0);
            double eff_i = 0, eff_h = 0, eff_3d = 0, weight_sum = 0;
            ...
            for (size_t i = 0; i < az_valid.size(); ++i) {
                ...
                // PROFESSOR'S CORRECTION: 3D Geometry
                ...
                eff_h += h_horiz * weights_valid[i];
                eff_3d += h_3d * weights_valid[i];
                weight_sum += weights_valid[i];
            }
            results_h[side] = eff_h / weight_sum;
            results_3d[side] = eff_3d / weight_sum;
            results_i[side] = eff_i / weight_sum;
            raw_flux_360[angle_deg + side * 180] = results_i[side];
        }
    ...
    // --- SMOOTHING AND DETERMINISTIC EXPECTED VALUE (E) ---
    ...
    // ================= TESTING BIN SIZES =================
    printf("\n============================================================\n");
    printf("PHASE 2: CHI-SQUARE STATISTICAL ANALYSIS (BY BINS)\n");
    printf("============================================================\n");
    ...
    for (int bw : bin_sizes) {
        ...
        std::vector<double> mc_binned(num_bins, 0.0);

        // ADDED: Reset generator so MC noise is stable for each bin size tested
        ...
        for (int b = 0; b < num_bins; ++b) {
            ...
            for (int i = 0; i < bw; ++i) {
                if (b * bw + i < 180) { 
                    O_bin += real_counts_1deg[b * bw + i];
                }
            }
            
            double E_bin = mc_binned[b];
        ...
        df = std::max(1, df - 1); // subtract 1 for scaling the total count
        ...
        printf("RESULT FOR %dst: Chi2 = %.2f, DF = %d, RedChi2 = %.4f, P-Value = %e\n", bw, chi2, df, reduced_chi2, p_value);
    }
    if (out) fclose(out);
    ...
    printf("============================================================\n");
    for(int i = 0; i < 3 && i < final_report.size(); ++i) {
