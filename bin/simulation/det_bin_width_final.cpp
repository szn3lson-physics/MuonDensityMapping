...
const std::string DATA_PATH = "/home/kacper/MuonDensityMapping/bin/data_analysis/output/selected_series/series_7_8_9_10/coin_34f.txt"; 

const std::string OUTPUT_FOLDER = "/home/kacper/MuonDensityMapping/bin/simulation/simulation_output/bin_analysis/instant/";
//const std::string OUTPUT_FOLDER = "/home/kacper/MuonDensityMapping/bin/simulation/simulation_output/bin_analysis/exp/";
//const std::string OUTPUT_FOLDER = "/home/kacper/MuonDensityMapping/bin/simulation/simulation_output/bin_analysis/gauss/";

const std::string OUTPUT_RESULTS = OUTPUT_FOLDER + "bin_analysis.txt";
const std::string OUTPUT_BEST_ANGLE_ANALYSIS = OUTPUT_FOLDER + "best_binsize_angle_analysis.txt";

const double TEST_RADIUS_M = 30.0;    // Promień testowy dla analizy binów
const double MAX_MAP_DEPTH_M = 100.0; // Maksymalny zasięg mapy dla ray-tracingu
...
int main() {
    ...
    printf("============================================================\n");
    printf("PHASE 1: THEORETICAL CALCULATIONS (R = %.1fm | Mode: %s)\n", TEST_RADIUS_M, mode_str.c_str());
    printf("============================================================\n");
    ...
    // Stały krok skanowania, aby geometria detektora była niezależna od R
    double step_az_el = PIXEL_SIZE / MAX_MAP_DEPTH_M;
    ...
    int max_map_steps = static_cast<int>(std::round(MAX_MAP_DEPTH_M / STEP_SIZE_M));
    int bar_width = 50;
            ...
            for (size_t i = 0; i < az_valid.size(); ++i) {
                ...
                for (int s = 0; s <= max_map_steps; ++s) {
                    ...
                    if (is_rock) {
                        ...
                        // ZASTOSOWANIE WYBRANEGO MODELU WYGASZANIA PUSTEJ PRZESTRZENI
                        double visibility_weight = 0.0;
                        if (TEST_RADIUS_M > 0.0) {
                            if (ATTENUATION_MODE == 0) {
                                visibility_weight = (r <= TEST_RADIUS_M) ? 1.0 : 0.0;
                            } else if (ATTENUATION_MODE == 1) {
                                visibility_weight = std::exp(-r / TEST_RADIUS_M);
                            } else if (ATTENUATION_MODE == 2) {
                                visibility_weight = std::exp(-(r * r) / (TEST_RADIUS_M * TEST_RADIUS_M));
                            }
                        }
                        h_horiz += STEP_SIZE_M * (1.0 - visibility_weight);
                    }
                }

                // POPRAWKA FIZYCZNA: Slant depth bez cos^2(theta)
    ...
    // --- POPRAWKA: USUNIĘTO PODWÓJNE WYGŁADZANIE BARTLETTEM ---
    std::vector<double> expected_1deg(180, 0.0);
    for (int i = 0; i < 180; ++i) {
        // Dodajemy strumień z przodu i z tyłu detektora
