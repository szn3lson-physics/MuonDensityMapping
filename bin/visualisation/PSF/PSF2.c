...
const char* OUTPUT_NAME = "/home/kacper/MuonDensityMapping/figures/PSF_2_Detectors_Combined.pdf";
...
const double MAX_DIST   = 70.0; // Promień zasięgu detektora
...
void PSF2() {
    ...
    for (int ix = 1; ix <= RES; ++ix) {
        ...
        for (int iy = 1; iy <= RES; ++iy) {
            ...
            // Rysowanie bez ucinania poza MAX_DIST (jak w najnowszym PSF3)
            h_det1->SetBinContent(ix, iy, n1);
            h_det2->SetBinContent(ix, iy, n2);

            bool a1 = (dist1 >= MIN_DIST), a2 = (dist2 >= MIN_DIST);

            // Superpozycja
            h_overlap->SetBinContent(ix, iy, (a1?n1:0) + (a2?n2:0));

            // Triangulacja
            if (a1 && a2) h_corr->SetBinContent(ix, iy, n1 * n2);
    ...
    // ════════════════════════════════════════════════════════════════════════
    //  COMBINED CANVAS (Siatka 3x2 identyczna z PSF3)
    // ════════════════════════════════════════════════════════════════════════
    TCanvas* c = new TCanvas("c", "Muon Tomography PSF 2-Detectors", 1600, 1000);
    ...
    // Pad 3 zostaje pusty, by układ idealnie pokrywał się z tym co masz w PSF3

    c->cd(4); gPad->SetRightMargin(0.15); h_overlap->Draw("COLZ"); draw_decorations(true, true);
    c->cd(5); gPad->SetRightMargin(0.15); h_corr->Draw("COLZ");    draw_decorations(true, true);
    c->cd(6); gPad->SetRightMargin(0.15); h_recon->Draw("COLZ");   draw_decorations(true, true);
