...
// For Triangulation and Reconstruction
...
// --- NEW SPECIFIC FOR 4TH PLOT (Sharp contrast for superposition) ---
void apply_extreme_contrast(TH2F* h) {
    ...
    // which pushes the entire blurred background to black and highlights only the peaks
...
void PSF3() {
    ...
    double cx = (std::min({D1X,D2X,D3X}) + std::max({D1X,D2X,D3X})) / 2.0;
    double cy = (std::min({D1Y,D2Y,D3Y}) + std::max({D1Y,D2Y,D3Y})) / 2.0;
    double half_span = map_dist + std::max({
        std::abs(D1X - cx), std::abs(D2X - cx), std::abs(D3X - cx),
        std::abs(D1Y - cy), std::abs(D2Y - cy), std::abs(D3Y - cy)
    }) + 2.0;
    ...
    auto dens3 = get_angular_density("D:\\MuonDensityMapping\\sim_4_root_events.txt", spread_deg);
    ...
    TH2F* h_det3 = new TH2F("h_det3","Detector 3;X [m];Y [m]",                  RES,Xs1,Xs2, RES,Ys1,Ys2);
    
    // Superposition (Sum of signals covering the entire map)
    TH2F* h_overlap  = new TH2F("h_overlap", "Superposition of 3 det.;X [m];Y [m]",  RES,Xc1,Xc2, RES,Yc1,Yc2);
    ...
    for (int ix = 1; ix <= RES; ++ix) {
        ...
        for (int iy = 1; iy <= RES; ++iy) {
            ...
            h_det3->SetBinContent(ix, iy, dens3[idx]);
        }
    }
    ...
    for (int ix = 1; ix <= RES; ++ix) {
        ...
        for (int iy = 1; iy <= RES; ++iy) {
            ...
            double o3_full = (dist3 >= min_dist) ? n3 : 0.0;
            h_overlap->SetBinContent(ix, iy, o1_full + o2_full + o3_full);

            // Triangulation (on the whole map)
            if ((dist1 >= min_dist) && (dist2 >= min_dist) && (dist3 >= min_dist))
                h_corr->SetBinContent(ix, iy, n1 * n2 * n3);
            else
    ...
    // <---- NEW FORMATTING FOR 4TH PLOT ---->
    apply_extreme_contrast(h_overlap); 
    
    apply_dynamic_scale_map(h_corr);

    // ════════════════════════════════════════════════════════════════════════
    //  CANVAS 1: PSF 3 detectors + Superposition (Sum of the full map)
    // ════════════════════════════════════════════════════════════════════════
    TCanvas* c1 = new TCanvas("c1", "PSF 3 detectors", 1120, 1120);
    ...
    c1->cd(4); gPad->SetLeftMargin(0.13); gPad->SetRightMargin(0.13); gPad->SetBottomMargin(0.11); gPad->SetTopMargin(0.09);
    h_overlap->Draw("COLZ");
    draw_yellow_circle(D1X, D1Y, max_dist);
    draw_yellow_circle(D2X, D2Y, max_dist);
    draw_yellow_circle(D3X, D3Y, max_dist);
    draw_det(D1X, D1Y);
    draw_det(D2X, D2Y);
    draw_det(D3X, D3Y);

    c1->SaveAs("D:\\MuonDensityMapping\\figures\\PSF_3_detectors.pdf");
    ...
    c2->SaveAs("D:\\MuonDensityMapping\\figures\\PSF_3_binary_map.pdf");

    std::cout << "Finished. Files PSF_detectors.pdf and PSF_3.pdf saved." << std::endl;
}