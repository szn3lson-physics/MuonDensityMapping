...
void his1_time() {
    ...
    // ==========================================================
    std::string input_file = "/home/kacper/MuonDensityMapping/output/all/zussamen_10/coin_time.txt";
    int bin_width_deg = 1;       // Domyślnie 9 stopni na bin
    double manual_sigma = 3.0;   // Ile sigm chcesz narysować (np. 3.0)
    // ==========================================================
    ...
    std::string line;
    std::getline(file, line); // Pomijamy nagłówek "Angle;Time_Minutes;Counts_34f"
    ...
    while (std::getline(file, line)) {
        ...
        // Zamiana kąta 1-180 na index binu
    ...
    // -----------------------------
    TH1F *hist = new TH1F("hist", Form("Detector Data (Bin: %d#circ);Zenith Angle [#circ];Muon Rate [Counts / Hour]", bin_width_deg), n_bins, 1, 180);
    ...
    for (int i = 0; i < n_bins; ++i) {
        ...
        if (sum_time_min[i] > 0) {
            ...
            rate = sum_counts[i] / time_hours; // Wyliczanie Rate (zliczenia na godzinę)
            
            global_counts += sum_counts[i];
            global_time_hours += time_hours;
        }
        hist->SetBinContent(i + 1, rate); // W ROOT biny numerowane są od 1
        total_rate += rate;
    }
    ...
    // Statystycznie poprawny błąd Poissona dla częstości (Rate)
    ...
    double sigma = manual_sigma * sigma_1;

    // -----------------------------
    //   4. Apply global style
    // -----------------------------
    gStyle->SetOptStat(0);       // Brak standardowej tabelki statystyk
    ...
    hist->SetLineWidth(2);
    hist->SetFillColorAlpha(kRed, 0.3); // Przezroczyste wypełnienie
    hist->SetStats(0);
    ...
    // Skalowanie osi Y aby zmieścić linie sigmy
    ...
    mean_y_line->SetLineStyle(2); // Dotted
    ...
    mean_y_text->DrawLatex(x_max * 0.95, mean_y * 1.05, Form("Mean Rate: %.1f / h", mean_y));
